...
--DROP EVENT SESSION [SP_EXEC_HISTORY] ON SERVER

CREATE EVENT SESSION [SP_EXEC_HISTORY] ON SERVER 
ADD EVENT sqlserver.sp_statement_completed(
	ACTION(sqlserver.client_app_name, 
		...
		sqlserver.sql_text,
		sqlos.task_time)
    WHERE (([object_type]=(8272)) AND ([source_database_id]=(5)))
	--WHERE ([sqlserver].[like_i_sql_unicode_string]([sqlserver].[sql_text], N'%Unable to retrieve SQL text%'))
)
ADD TARGET package0.asynchronous_file_target (
	SET FILENAME = N'E:\SQLExtendedEvents\SPExecutionHistory.xel',
	--max_file_size = (10),
	METADATAFILE = N'E:\SQLExtendedEvents\SPExecutionHistory.xem'
)
...
ALTER EVENT SESSION [SP_EXEC_HISTORY] ON SERVER
  STATE = START;
GO
ALTER EVENT SESSION [SP_EXEC_HISTORY] ON SERVER
STATE = STOP;
...
(  SELECT data = CONVERT(XML, event_data)
   ...
   'E:\SQLExtendedEvents\SPExecutionHistory*.xel', 
   'E:\SQLExtendedEvents\SPExecutionHistory*.xem', 
   NULL, NULL )
),
tab AS (  SELECT data,
